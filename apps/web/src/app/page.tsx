...
import { SEVERITY_CHIP, SOURCE_META, SOURCE_ORDER, VERDICT_CHIP } from "@/lib/ui";
...
const VERDICT_BANDS = [
  { verdict: "pass" as const, range: "85 – 100", meaning: "Nothing blocking from the checks that ran." },
  { verdict: "review" as const, range: "50 – 84", meaning: "Real issues, not immediately exploitable. Read them first." },
  { verdict: "block" as const, range: "0 – 49", meaning: "At least one finding is exploitable as-is." },
];
...
export default async function Home() {
  ...
  const latestComplete = scans.find((scan) => scan.status === "done" && scan.verdict !== null) ?? null;
    ...
    <main>
      ...
      <Band tone="light">
        <div className="max-w-4xl">
          ...
          <h1 className="display-heading mt-7 text-4xl leading-[1.02] sm:text-6xl">
            ...
            <span className="relative inline-block">
              AI-generated code
              <span aria-hidden className="absolute -bottom-1 left-0 h-1.5 w-full bg-brand" />
            </span>
          </h1>

          <p className="mt-8 max-w-2xl text-base leading-relaxed text-fg-muted sm:text-lg">
            AI writes code faster than anyone can review it. VibeGuard runs three real security
            scanners over a public GitHub repo, scores it 0–100, and explains every finding in plain
            language with a fix you can paste straight back into your editor.
          </p>

          <div className="mt-10 flex flex-wrap gap-4">
            <Link href="/scan" className="brut-btn px-7 py-3.5 text-sm no-underline">
              Scan a repository
            </Link>
            <Link
        ...
        <ul className="mt-16 grid grid-cols-2 gap-4 lg:grid-cols-4">
          {STATS.map((stat) => (
            <li key={stat.label} className="brut border-2 border-line-strong p-5">
              <span aria-hidden className={`block h-2.5 w-8 ${stat.accent}`} />
              <div className="display-heading mt-3.5 text-3xl text-fg">{stat.value}</div>
      ...
      <Band labelledBy="capabilities-heading">
        ...
        <ul className="grid gap-5 lg:grid-cols-2">
          {CAPABILITIES.map((item) => (
            <li key={item.title} className="brut brut-hover border-2 border-line-strong p-6 sm:p-7">
              <span aria-hidden className={`block h-8 w-8 border-2 border-ink ${item.accent}`} />
          ...
          {PIPELINE.map((stage) => (
            <li key={stage.step} className="relative pb-6 last:pb-0">
              ...
              <div className="brut border-2 border-line-strong p-6">
                ...
                <p className="mt-3 text-sm leading-relaxed text-fg-muted">{stage.body}</p>
              </div>
            </li>
          ))}
        </ol>
      ...
      <Band id="scanners" labelledBy="scanners-heading">
        ...
        <ul className="grid gap-5 sm:grid-cols-2">
          {SOURCE_ORDER.map((source) => (
            <li key={source} className="brut brut-hover border-2 border-line-strong p-6 sm:p-7">
              <span
                aria-hidden
        ...
        <p className="mt-10 text-center text-sm text-fg-muted">
          <Link href="/docs/how-it-works#scanner-architecture" className="text-fg hover:text-brand">
          ...
          <div className="brut border-2 border-line-strong p-6 sm:p-7">
            ...
            <ul className="mt-5 grid gap-4">
              {VERDICT_BANDS.map((band) => (
                <li key={band.verdict} className="flex flex-wrap items-baseline gap-x-4 gap-y-1">
                  <span className={`chip ${VERDICT_CHIP[band.verdict]}`}>{band.verdict}</span>
                  <span className="font-mono text-sm tabular-nums text-fg">{band.range}</span>
          ...
          <div className="brut border-2 border-line-strong p-6 sm:p-7">
            ...
            <ul className="mt-5 flex flex-wrap gap-2.5">
              {SEVERITY_ORDER.map((severity) => (
                <li key={severity}>
                  <span className={`chip ${SEVERITY_CHIP[severity]}`}>{severity}</span>
                </li>
              ))}
            </ul>
            <p className="mt-5 text-sm leading-relaxed text-fg-muted">
              A critical costs far more than a low, and four instances of the same rule cost less
              than four different criticals — otherwise one repeated pattern would saturate the
              score and hide everything underneath it.
            </p>
            <Link
      ...
      <Band tone="light" labelledBy="trust-heading">
        ...
        <ul className="grid gap-5 lg:grid-cols-2">
          {PRINCIPLES.map((principle) => (
            <li key={principle.title} className="brut relative border-2 border-line-strong p-6 sm:p-7">
              <CornerMarks />
      ...
      {/* ── 8 · BRAND — closing CTA ─────────────────────────────────────── */}
      <CtaBand secondary={{ href: "/docs/getting-started", label: "Getting started" }} />
    </main>
  );
}
